  ...
  'inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50',
  {
    variants: {
      variant: {
        ...
        calm: 'bg-calm-600 text-white hover:bg-calm-700',
 ...
 * The loading spinner is NOT rendered when asChild=true to prevent
 * React.Children.only errors.
 */
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading, children, disabled, ...props }, ref) => {
    // When asChild is true, render Slot with the single child element.
    // Never inject extra children (spinner) — Slot requires exactly one.
    if (asChild) {
      return (
        <Slot
    ...
    // Standard button element — safe to render multiple children
    return (
      <button
        className={cn(buttonVariants({ variant, size, className }))}
        ref={ref}
