  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">דוחות ואנליטיקה</h1>
          <p className="text-slate-400 mt-1">
            נתוני ביצועים ומדדי צמיחה של הפלטפורמה
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
          <p className="text-sm text-slate-400">סה״כ מטופלים</p>
          <p className="text-3xl font-bold text-white mt-2">
            {isLoading ? '...' : reports?.totalPatients ?? 0}
          </p>
        </div>
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
          <p className="text-sm text-slate-400">מטפלים פעילים</p>
          <p className="text-3xl font-bold text-white mt-2">
            {isLoading ? '...' : reports?.approvedTherapists ?? 0}
          </p>
        </div>
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
          <p className="text-sm text-slate-400">סה״כ פגישות</p>
          <p className="text-3xl font-bold text-white mt-2">
            {isLoading ? '...' : reports?.totalSessions ?? 0}
          </p>
        </div>
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
          <p className="text-sm text-slate-400">שיעור התאמה</p>
          <p className="text-3xl font-bold text-white mt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Session Stats */}
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-white mb-4">סטטיסטיקת פגישות</h2>
          {isLoading ? (
                  ...
                  <>
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-slate-400">מאושרות</span>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-slate-400">הושלמו</span>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-slate-400">ממתינות</span>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-slate-400">בוטלו</span>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-white mb-4">התפלגות קופות חולים</h2>
          {isLoading ? (
            <p className="text-slate-400">טוען...</p>
        ...
        {/* Top Therapists */}
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-white mb-4">מטפלים מובילים</h2>
          {isLoading ? (
            <p className="text-slate-400">טוען...</p>
            ...
            <div className="space-y-3">
              {reports.topTherapists.map((therapist, index) => (
                ...
                >
                  ...
                  <span className="text-slate-400 text-sm">{therapist.sessions} פגישות</span>
                </div>
              ))}
            </div>
          ) : (
      ...
      {/* Export Options */}
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
        <h2 className="text-lg font-semibold text-white mb-4">ייצוא דוחות</h2>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <button className="p-4 bg-slate-700 rounded-lg hover:bg-slate-600 transition-colors text-right">
            <p className="text-white font-medium">דוח משתמשים</p>
            <p className="text-slate-400 text-sm mt-1">כל המשתמשים עם סטטוס ופעילות</p>
          </button>
          <button className="p-4 bg-slate-700 rounded-lg hover:bg-slate-600 transition-colors text-right">
            <p className="text-white font-medium">דוח פגישות</p>
            <p className="text-slate-400 text-sm mt-1">היסטוריית פגישות וסטטיסטיקה</p>
          </button>
          <button className="p-4 bg-slate-700 rounded-lg hover:bg-slate-600 transition-colors text-right">
            <p className="text-white font-medium">דוח פיננסי</p>
            <p className="text-slate-400 text-sm mt-1">הכנסות ונתוני חיוב</p>
          </button>
          <button className="p-4 bg-slate-700 rounded-lg hover:bg-slate-600 transition-colors text-right">
            <p className="text-white font-medium">דוח ציות</p>
