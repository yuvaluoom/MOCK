'use client';

import Link from 'next/link';

export default function AboutPage() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-trust-50/30 to-white py-20">
      <div className="container mx-auto px-4 max-w-3xl">
        <h1 className="text-4xl font-bold text-gray-900 mb-6">אודות MatchMind</h1>
        <div className="bg-white rounded-2xl shadow-sm p-8 text-gray-600 leading-relaxed space-y-4">
          <p>
            MatchMind היא פלטפורמה חכמה להתאמת מטופלים ומטפלים בתחום בריאות הנפש.
          </p>
          <p>
            אנחנו מאמינים שהתאמה נכונה בין מטופל למטפל היא המפתח להצלחת הטיפול, ולכן פיתחנו מערכת מבוססת מדע שמזהה את ההתאמה הטובה ביותר.
          </p>
          <p className="text-sm text-muted-foreground">העמוד בבנייה.</p>
        </div>
        <Link href="/" className="inline-block mt-8 text-trust-600 hover:text-trust-700 font-medium transition-colors">
          &larr; חזרה לדף הבית
        </Link>
      </div>
    </div>
  );
}
